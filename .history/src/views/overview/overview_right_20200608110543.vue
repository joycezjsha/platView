  ...
  <div class="overview-info">
    <div class="overview-info_container">
      ...
      <div class="boxstyle">
        ...
        <m-line-chart :chart_data="chart_data" c_id='accurCreateChange' style='width:100%;height:18vh'></m-line-chart>
      </div>
    </div>
  </div>
</template>
    ...
    initSumCharts(){
      ...
      .then(response => {
          if (response && response.status == 200) {
            ...
            if (data.errcode == 0) {
              debugger;
             _this.chart_data.timelist=data.data.timelist;
             data.data.inlist.forEach((e,index)=>{
               _this.chart_data.y1data.push([_this.chart_data.timelist[index],e]);
             });
             data.data.outlist.forEach((e,index)=>{
               _this.chart_data.y2data.push([_this.chart_data.timelist[index],e]);
             });
            } else {
              that.$message({
                message: data.errmsg,
