  ...
  <div class="overview-info">
    <div class="overview-info_container">
      ...
      <div class="boxstyle">
        ...
        <bar-chart c_id='accurCreateChange' :chart_data="car_chart_data" style='width:100%;height:28vh'></bar-chart>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initAccurCharts(){
      let _this=this;
      interf.GET_CAR_FLOW_API({stime:1})
      .then(response => {
          if (response && response.status == 200) {
            var data = response.data;
            if (data.errcode == 0) {
              let _data=data.data;
              _this.carStatics.count=_data.inventory?data.inventory:'';
              let _data=_this.car_chart_data;
              for(let key in _data.data){
                _data.xdata.push(key);
                _data.ydata.push(_data.data[key].inventory);
              }
             _this.car_chart_data=_data;
            } else {
              _this.$message({
