      ...
      .then(response => {
          if (response && response.status == 200) {
            ...
            if (data.errcode == 0) {
             ...
             _this.chart_data.y1data=data.data.inlist.forEach((e,index)=>{
               return [_this.chart_data.timelist[index],e];
             });
             _this.chart_data.y2data=data.data.outlist.forEach((e,index)=>{
               return [_this.chart_data.timelist[index],e];
             });
            } else {
              that.$message({
