  ...
  <div class="accident-statics">
    <div class="accident-statics_container">
      ...
      <div class="accident-statics_content">
        <div> <i class="el-icon-collection-tag">事故类型：</i>
            <el-row v-for="(item,index) in sg_sort_data" :key="index"><el-col :span="12">{{item.name}}</el-col><el-col :span="6">{{item.value}}起</el-col><el-col :span="6">{{item.radio}}</el-col></el-row>
        </div>
        <div>事故发生数量变化：
...
export default {
  ...
  data() {
    return {
      ...
      sg_sort_data:[{name:'机动车与机动车',value:'12',radio:'32%'},{name:'机动车与非机动车',value:'12起',radio:'32%'},{name:'行人',value:'2',radio:'32%'}]
    };
  },
  mounted() {
