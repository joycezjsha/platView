...
export default {
  data() {
    return {
      option: {
          // color:['rgba(243,200,71,1)','#05eef0'],
          grid:{
            top:'30%',
            left:'15%',
            right:'15%',
  ...
  methods: {
    ...
    initAccidentStaticsChart() {
      ...
      this.option.series=[];
      this.chart_data.ydata.forEach(e=>{
      ...
      if(this.chart_data.unit){
          this.option.yAxis.name=this.chart_data.legend[0]+'（单位：'+this.chart_data.unit[0]+')';
        }else{
          this.option.yAxis.name=this.chart_data.legend[0];
        }
